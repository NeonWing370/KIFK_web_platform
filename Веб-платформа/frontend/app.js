(function () {
  ...
  const TOKEN_KEY = "codestudy-token";
  ...
  document.addEventListener("DOMContentLoaded", function () {
    CodeStudyStorage.init().then(function (platformData) {
      data = platformData;
  ...
  function refreshData() {
    const latest = CodeStudyStorage.reload();
    if (latest) data = latest;
    return data;
  }

  function writeData() {
    CodeStudyStorage.save(data);
  }
  ...
  function courseMaterials(courseId) {
    return CodeStudyStorage.getMaterials(data, courseId || ELECTRONICS_ID);
  }
  ...
  function courseTeacherTests(courseId) {
    return CodeStudyStorage.getTeacherTests(data, courseId || ELECTRONICS_ID);
  }
  ...
  function syncCourseContent(courseId) {
    ...
    return Promise.all([
      ...
      const course = CodeStudyStorage.ensureCourseCollections(data, courseId);
      if (!course) return;
  ...
  function electronicsCourse() {
    return CodeStudyStorage.ensureCourseCollections(data, ELECTRONICS_ID);
  }
  ...
  function setupPlatformDataPage() {
    ...
    if (downloadButton) {
      downloadButton.addEventListener("click", function () {
        CodeStudyStorage.downloadJson(data);
      });
    }
  }
  ...
  function setupMaterialQuiz(form) {
    ...
    const saved = JSON.parse(localStorage.getItem("codestudyMaterialScores") || "{}");
    ...
    form.addEventListener("submit", function (event) {
      ...
      saved[key] = score;
      localStorage.setItem("codestudyMaterialScores", JSON.stringify(saved, null, 2));
