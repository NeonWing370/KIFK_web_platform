<template>
  <div class="course-shell electronics-course electric-course-page microprocessor-systems-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          <h1>Мікропроцесорні системи</h1>
          <p class="subtitle">
            Архітектура мікропроцесорів, контролери, порти та вбудовані системи.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите архітектуру мікропроцесорів, контролери, порти вводу-виводу та вбудовані системи.</li>
                </ul>
              </article>
            </div>
          </div>
...
const isJoined = computed(() => {
  return user.value.joinedCourses?.some((course) => {
    return (
      course.slug === "microprocessor-systems" ||
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/microprocessor-systems");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "microprocessor-systems";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/microprocessor-systems");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/microprocessor-systems/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "microprocessor-systems" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "microprocessor-systems" } };
}
...
.microprocessor-systems-course { --electric-orange: #fb8500; --electric-blue: #219ebc; }

.microprocessor-systems-course .electronics-grid { background-size: 18px 18px; opacity: 0.35; }
.microprocessor-systems-course .electronics-shape.chip { border-radius: 2px; border-width: 5px; box-shadow: 0 0 0 12px rgba(251, 133, 0, 0.08), 0 0 40px rgba(33, 158, 188, 0.32); animation: microprocessor-register-scan 5s linear infinite; }
.microprocessor-systems-course .processor-core span { background: #fb8500; animation-duration: 1.4s; }
@keyframes microprocessor-register-scan { 0%,100% { filter: brightness(0.8); } 45% { filter: brightness(1.45); } 55% { filter: brightness(0.85); } }


.microprocessor-systems-course,
.microprocessor-systems-course h1,
.microprocessor-systems-course h2,
.microprocessor-systems-course h3,
.microprocessor-systems-course h4,
.microprocessor-systems-course p,
.microprocessor-systems-course button,
.microprocessor-systems-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.microprocessor-systems-course .electronics-hero-visual { isolation: isolate; }
.microprocessor-systems-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.microprocessor-systems-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.microprocessor-systems-course { background: #1c1004; color: #fff7e8; }
.microprocessor-systems-course .course-signature { grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(4, 1fr); padding: 17%; gap: 5px; border: 3px solid #fb8500; box-shadow: 0 0 0 14px rgba(33, 158, 188, .12); }
.microprocessor-systems-course .course-signature span { background: #219ebc; opacity: .35; animation: microprocessor-register-write 2.7s steps(2, end) infinite; }
.microprocessor-systems-course .course-signature span:nth-child(4n) { animation-delay: .6s; background: #fb8500; }
.microprocessor-systems-course .course-tabs button { border-radius: 2px; }
.microprocessor-systems-course .content-panel { border-radius: 6px; }
@keyframes microprocessor-register-write { 50% { opacity: 1; box-shadow: 0 0 16px #fb8500; } }
