<template>
  <div class="course-shell electronics-course electric-course-page operating-systems-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          ...
          <p class="subtitle">
            Процеси, планування, пам'ять, файли та керування ресурсами операційної системи.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите процеси, планування, пам'ять, файлові системи та керування ресурсами ОС.</li>
                </ul>
              </article>
            </div>
          </div>
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/operating-systems");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "operating-systems";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/operating-systems");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/operating-systems/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "operating-systems" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "operating-systems" } };
}
...
.operating-systems-course { --electric-orange: #90be6d; --electric-blue: #43aa8b; }

.operating-systems-course .electronics-grid {
  background-size: 100% 56px;
  background-image: repeating-linear-gradient(0deg, rgba(144, 190, 109, 0.2) 0 2px, transparent 2px 56px);
  animation: operating-scheduler-shift 7s steps(7, end) infinite;
}
.operating-systems-course .electronics-shape.ring { border-radius: 8px; transform: rotate(0deg); }
.operating-systems-course .electronics-shape.chip { border-radius: 8px; transform: translateY(-12px); animation: operating-process-cycle 4s ease-in-out infinite; }
@keyframes operating-scheduler-shift { to { background-position: 0 56px; } }
@keyframes operating-process-cycle { 50% { transform: translateY(14px); filter: brightness(1.4); } }


.operating-systems-course,
.operating-systems-course h1,
.operating-systems-course h2,
.operating-systems-course h3,
.operating-systems-course h4,
.operating-systems-course p,
.operating-systems-course button,
.operating-systems-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.operating-systems-course .electronics-hero-visual { isolation: isolate; }
.operating-systems-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.operating-systems-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.operating-systems-course { background: #0c180e; color: #f4fff0; }
.operating-systems-course .course-signature { grid-template-columns: 1fr; align-content: center; padding: 12%; gap: 10px; }
.operating-systems-course .course-signature span { height: 20px; border-left: 5px solid #90be6d; background: linear-gradient(90deg, rgba(144,190,109,.75) 0 42%, rgba(67,170,139,.18) 42% 100%); animation: operating-process-run 4s linear infinite; }
.operating-systems-course .course-signature span:nth-child(3n + 2) { width: 78%; animation-delay: .65s; border-color: #43aa8b; }
.operating-systems-course .course-tabs button { border-radius: 8px 8px 0 0; }
.operating-systems-course .content-panel { border-radius: 8px 8px 2px 2px; }
@keyframes operating-process-run { 0% { transform: translateX(-18px); opacity: .25; } 45% { opacity: 1; } 100% { transform: translateX(18px); opacity: .25; } }
