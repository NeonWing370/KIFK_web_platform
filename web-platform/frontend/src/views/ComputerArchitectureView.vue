<template>
  <div class="course-shell electronics-course electric-course-page computer-architecture-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          ...
          <p class="subtitle">
            Будова процесора, пам'ять, шини даних та принципи роботи комп'ютерних систем.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите архітектуру процесора, пам'ять, шини даних та взаємодію основних вузлів комп'ютера.</li>
                </ul>
              </article>
            </div>
          </div>
...
const isJoined = computed(() => {
  return user.value.joinedCourses?.some((course) => {
    return (
      course.slug === "computer-architecture" ||
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/computer-architecture");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "computer-architecture";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/computer-architecture");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/computer-architecture/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "computer-architecture" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "computer-architecture" } };
}
...
.computer-architecture-course { --electric-orange: #f9c74f; --electric-blue: #f9844a; }

.computer-architecture-course .electronics-grid {
  background-size: 64px 32px;
  background-image: linear-gradient(rgba(249, 199, 79, 0.2) 1px, transparent 1px), linear-gradient(90deg, rgba(249, 132, 74, 0.14) 1px, transparent 1px);
  animation: architecture-blueprint-shift 12s linear infinite;
}
.computer-architecture-course .trace { height: 4px; border-radius: 0; box-shadow: 0 0 14px #f9c74f; animation-duration: 3s; }
.computer-architecture-course .processor-core { border-radius: 4px; transform: rotate(0deg); }
@keyframes architecture-blueprint-shift { to { background-position: 64px 32px, 64px 32px; } }


.computer-architecture-course,
.computer-architecture-course h1,
.computer-architecture-course h2,
.computer-architecture-course h3,
.computer-architecture-course h4,
.computer-architecture-course p,
.computer-architecture-course button,
.computer-architecture-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.computer-architecture-course .electronics-hero-visual { isolation: isolate; }
.computer-architecture-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.computer-architecture-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.computer-architecture-course { background: #17120a; color: #fff9e8; }
.computer-architecture-course .course-signature { grid-template-rows: repeat(6, 1fr); padding: 18% 6%; gap: 11px; }
.computer-architecture-course .course-signature span { height: 4px; align-self: center; background: linear-gradient(90deg, transparent, #f9c74f 18%, #f9844a 82%, transparent); box-shadow: 0 0 15px rgba(249, 199, 79, 0.58); animation: architecture-bus-transfer 3.2s ease-in-out infinite; }
.computer-architecture-course .course-signature span:nth-child(even) { animation-delay: 0.55s; transform-origin: right; }
.computer-architecture-course .course-tabs button, .computer-architecture-course .content-panel { border-radius: 0; }
@keyframes architecture-bus-transfer { 0%,100% { transform: scaleX(0.22); opacity: 0.32; } 50% { transform: scaleX(1); opacity: 1; } }
