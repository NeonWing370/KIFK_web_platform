<template>
  <div class="course-shell electronics-course electric-course-page computer-circuitry-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          <h1>Комп'ютерна схемотехніка</h1>
          <p class="subtitle">
            Цифрові вузли, логічні схеми, тригери та проєктування електронних пристроїв.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              <article class="content-panel wide-panel">
                ...
                <h3>Комп'ютерна схемотехніка</h3>
                <p>
                  Цифрові вузли, логічні схеми, тригери та проєктування електронних пристроїв.
                </p>
              </article>
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите цифрові вузли, логічні схеми, тригери та принципи проєктування комп'ютерних пристроїв.</li>
                </ul>
              </article>
            </div>
          </div>
...
const isJoined = computed(() => {
  return user.value.joinedCourses?.some((course) => {
    return (
      course.slug === "computer-circuitry" ||
      course === "computer-circuitry" ||
      course.title === "Computer Circuitry"
    );
  });
});
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/computer-circuitry");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "computer-circuitry";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/computer-circuitry");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/computer-circuitry/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "computer-circuitry" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "computer-circuitry" } };
}
...
.computer-circuitry-course { --electric-orange: #00f5d4; --electric-blue: #9b5de5; }

.computer-circuitry-course .electronics-grid {
  background-size: 36px 36px;
  background-image: linear-gradient(90deg, rgba(0, 245, 212, 0.16) 2px, transparent 2px), linear-gradient(rgba(155, 93, 229, 0.16) 2px, transparent 2px);
  animation: circuitry-gate-step 6s steps(6, end) infinite;
}
.computer-circuitry-course .electronics-shape.ring { border-radius: 12px 42px 12px 42px; }
.computer-circuitry-course .node { border-radius: 2px; animation: circuitry-node-toggle 2.4s steps(2, end) infinite; }
@keyframes circuitry-gate-step { to { background-position: 36px 0, 0 36px; } }
@keyframes circuitry-node-toggle { 50% { opacity: 0.18; transform: scale(0.72); } }


.computer-circuitry-course,
.computer-circuitry-course h1,
.computer-circuitry-course h2,
.computer-circuitry-course h3,
.computer-circuitry-course h4,
.computer-circuitry-course p,
.computer-circuitry-course button,
.computer-circuitry-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.computer-circuitry-course .electronics-hero-visual { isolation: isolate; }
.computer-circuitry-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.computer-circuitry-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.computer-circuitry-course { background: #071811; color: #edfffb; }
.computer-circuitry-course .course-signature { grid-template-columns: repeat(3, 1fr); padding: 13%; gap: 12px; }
.computer-circuitry-course .course-signature span { border: 2px solid #00f5d4; border-radius: 20px 4px 20px 4px; background: rgba(155, 93, 229, .14); animation: circuitry-gate-switch 2.4s steps(2, end) infinite; }
.computer-circuitry-course .course-signature span:nth-child(2n) { animation-delay: .8s; border-color: #9b5de5; }
.computer-circuitry-course .course-tabs button { border-radius: 4px 18px 4px 18px; }
.computer-circuitry-course .content-panel { border-radius: 4px; }
@keyframes circuitry-gate-switch { 50% { background: rgba(0, 245, 212, .48); box-shadow: 0 0 22px rgba(0, 245, 212, .58); } }
