<template>
  <div class="course-shell electronics-course electric-course-page peripheral-devices-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          <h1>Периферійні пристрої</h1>
          <p class="subtitle">
            Пристрої введення, виведення, інтерфейси та підключення комп'ютерного обладнання.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите пристрої введення і виведення, інтерфейси підключення та принципи роботи периферійного обладнання.</li>
                </ul>
              </article>
            </div>
          </div>
...
const isJoined = computed(() => {
  return user.value.joinedCourses?.some((course) => {
    return (
      course.slug === "peripheral-devices" ||
      course === "peripheral-devices" ||
      course.title === "Peripheral Devices"
    );
  });
});
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/peripheral-devices");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "peripheral-devices";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/peripheral-devices");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/peripheral-devices/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "peripheral-devices" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "peripheral-devices" } };
}
...
.peripheral-devices-course { --electric-orange: #4cc9f0; --electric-blue: #4895ef; }

.peripheral-devices-course .electronics-grid {
  background-size: 48px 48px;
  background-image: repeating-linear-gradient(90deg, rgba(76, 201, 240, 0.16) 0 1px, transparent 1px 48px), repeating-linear-gradient(0deg, rgba(72, 149, 239, 0.12) 0 1px, transparent 1px 48px);
  animation: peripheral-port-scan 8s linear infinite;
}
.peripheral-devices-course .electronics-shape.chip { border-radius: 8px; transform: rotate(0deg); border-width: 3px; }
.peripheral-devices-course .electronics-shape.pulse { width: 16rem; height: 3.5rem; border-radius: 6px; }
@keyframes peripheral-port-scan { 0%,100% { background-position: 0 0, 0 0; } 50% { background-position: 48px 0, 0 48px; } }


.peripheral-devices-course,
.peripheral-devices-course h1,
.peripheral-devices-course h2,
.peripheral-devices-course h3,
.peripheral-devices-course h4,
.peripheral-devices-course p,
.peripheral-devices-course button,
.peripheral-devices-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.peripheral-devices-course .electronics-hero-visual { isolation: isolate; }
.peripheral-devices-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.peripheral-devices-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.peripheral-devices-course { background: #071521; color: #effaff; }
.peripheral-devices-course .course-signature { grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(3, 1fr); padding: 12%; }
.peripheral-devices-course .course-signature span { border: 2px solid #4cc9f0; background: rgba(76, 201, 240, 0.12); box-shadow: inset 0 0 0 5px rgba(72, 149, 239, 0.12); animation: peripheral-socket-live 2.8s steps(2, end) infinite; }
.peripheral-devices-course .course-signature span:nth-child(3n) { animation-delay: 0.7s; border-color: #4895ef; }
.peripheral-devices-course .course-tabs button, .peripheral-devices-course .content-panel { border-radius: 3px; }
@keyframes peripheral-socket-live { 50% { background: rgba(76, 201, 240, 0.7); box-shadow: 0 0 22px rgba(76, 201, 240, 0.65); } }
