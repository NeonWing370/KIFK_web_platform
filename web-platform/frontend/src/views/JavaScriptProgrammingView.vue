<template>
  <div class="course-shell electronics-course electric-course-page javascript-programming-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          <h1>Програмування на JavaScript</h1>
          <p class="subtitle">
            Мова JavaScript, веб-інтерфейси, події та сучасна розробка застосунків.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              <article class="content-panel wide-panel">
                ...
                <h3>Програмування на JavaScript</h3>
                <p>
                  Мова JavaScript, веб-інтерфейси, події та сучасна розробка застосунків.
                </p>
              </article>
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите синтаксис JavaScript, функції, об'єкти, DOM та створення інтерактивних веб-застосунків.</li>
                </ul>
              </article>
            </div>
          </div>
...
const isJoined = computed(() => {
  return user.value.joinedCourses?.some((course) => {
    return (
      course.slug === "javascript-programming" ||
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/javascript-programming");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "javascript-programming";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/javascript-programming");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/javascript-programming/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "javascript-programming" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "javascript-programming" } };
}
...
.javascript-programming-course { --electric-orange: #f7df1e; --electric-blue: #61dafb; }

.javascript-programming-course .electronics-grid {
  background-size: 22px 22px;
  background-image: radial-gradient(circle, rgba(247, 223, 30, 0.45) 1px, transparent 1.5px);
  animation: javascript-event-loop 4s steps(8, end) infinite;
}
.javascript-programming-course .electronics-shape.ring { border-radius: 6px; transform: rotate(0deg); border-color: #f7df1e; }
.javascript-programming-course .electronics-shape.chip { clip-path: polygon(0 0, 84% 0, 100% 32%, 100% 100%, 16% 100%, 0 68%); }
@keyframes javascript-event-loop { 0%,100% { opacity: 0.3; } 50% { opacity: 0.9; background-position: 22px 22px; } }


.javascript-programming-course,
.javascript-programming-course h1,
.javascript-programming-course h2,
.javascript-programming-course h3,
.javascript-programming-course h4,
.javascript-programming-course p,
.javascript-programming-course button,
.javascript-programming-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.javascript-programming-course .electronics-hero-visual { isolation: isolate; }
.javascript-programming-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.javascript-programming-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.javascript-programming-course { background: #17170b; color: #fffdec; }
.javascript-programming-course .course-signature { grid-template-columns: repeat(3, 1fr); align-content: center; padding: 15%; gap: 14px; }
.javascript-programming-course .course-signature span { position: relative; height: 34px; border-left: 4px solid #f7df1e; background: rgba(97, 218, 251, 0.1); animation: javascript-queue-pop 3.6s cubic-bezier(.65,0,.35,1) infinite; }
.javascript-programming-course .course-signature span::after { content: "{}"; position: absolute; right: 8px; top: 3px; color: #61dafb; font: 800 18px/1 ui-monospace, monospace; }
.javascript-programming-course .course-signature span:nth-child(3n + 2) { animation-delay: .55s; }
.javascript-programming-course .course-tabs button { border-radius: 999px; }
.javascript-programming-course .content-panel { border-radius: 10px; }
@keyframes javascript-queue-pop { 0%,100% { transform: translateY(0); opacity: .34; } 35% { transform: translateY(-11px); opacity: 1; } 70% { transform: translateY(4px); opacity: .64; } }
