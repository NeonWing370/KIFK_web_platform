<template>
  <div class="course-shell electronics-course electric-course-page c-programming-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          <h1>Програмування мовою C</h1>
          <p class="subtitle">
            Синтаксис C, алгоритми, робота з пам'яттю та основи системного коду.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите синтаксис мови C, алгоритми, вказівники, масиви та роботу з пам'яттю.</li>
                </ul>
              </article>
            </div>
          </div>
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/c-programming");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "c-programming";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/c-programming");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/c-programming/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "c-programming" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "c-programming" } };
}
...
.c-programming-course { --electric-orange: #5dade2; --electric-blue: #af7ac5; }

.c-programming-course .electronics-grid {
  background-size: 100% 8px;
  background-image: repeating-linear-gradient(0deg, rgba(93, 173, 226, 0.12) 0 1px, transparent 1px 8px);
  animation: c-terminal-scan 5s linear infinite;
}
.c-programming-course .electronics-shape.ring { border-radius: 2px; border-color: #5dade2; transform: rotate(0deg); }
.c-programming-course .electronics-shape.pulse { border-radius: 0; width: 78%; height: 2px; }
@keyframes c-terminal-scan { to { background-position: 0 64px; } }


.c-programming-course,
.c-programming-course h1,
.c-programming-course h2,
.c-programming-course h3,
.c-programming-course h4,
.c-programming-course p,
.c-programming-course button,
.c-programming-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.c-programming-course .electronics-hero-visual { isolation: isolate; }
.c-programming-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.c-programming-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.c-programming-course { background: #07101d; color: #eff7ff; }
.c-programming-course .course-signature { grid-template-columns: 1fr; align-content: center; padding: 13%; gap: 8px; background: rgba(0, 0, 0, .34); border: 1px solid #5dade2; }
.c-programming-course .course-signature span { position: relative; height: 20px; border-left: 3px solid #5dade2; background: linear-gradient(90deg, rgba(93,173,226,.58) 0 54%, transparent 54%); animation: c-memory-read 2.9s steps(4, end) infinite; }
.c-programming-course .course-signature span::after { content: "0xC0DE"; position: absolute; right: 8px; top: 1px; color: #af7ac5; font: 700 12px/1.3 ui-monospace, monospace; }
.c-programming-course .course-signature span:nth-child(2n) { width: 72%; animation-delay: .5s; }
.c-programming-course .course-tabs button, .c-programming-course .content-panel { border-radius: 2px; }
@keyframes c-memory-read { 50% { filter: brightness(1.65); transform: translateX(8px); } }
