<template>
  <div class="course-shell electronics-course electric-course-page electric-magnetic-circuits-course">
    ...
    <main>
      <section class="hero template-course-hero">
        <div class="hero-content">
          <h1>Теорія електричних і магнітних кіл</h1>
          <p class="subtitle">
            Закони електричних кіл, магнітні поля, струм, напруга та аналіз схем.
          </p>
        </div>
          ...
          <div v-if="activeTab === 'overview'" class="tab-panel active">
            <div class="course-overview-grid">
              <article class="content-panel wide-panel">
                ...
                <h3>Теорія електричних і магнітних кіл</h3>
                <p>
                  Закони електричних кіл, магнітні поля, струм, напруга та аналіз схем.
                </p>
              </article>
              ...
              <article class="content-panel">
                <h3>Деталі</h3>
                <ul>
                  <li>Тут ви вивчите закони електричних і магнітних кіл, струм, напругу, опір та методи аналізу схем.</li>
                </ul>
              </article>
            </div>
          </div>
...
const isJoined = computed(() => {
  return user.value.joinedCourses?.some((course) => {
    return (
      course.slug === "electric-magnetic-circuits" ||
      course === "electric-magnetic-circuits" ||
      course.title === "Theory of Electric and Magnetic Circuits"
    );
  });
});
...
onMounted(async () => {
  try {
    ...
    try {
      const testsRes = await api.get("/tests/course/electric-magnetic-circuits");
    ...
    try {
      ...
      results.value = resultsRes.data.filter((result) => {
        return result.course?.slug === "electric-magnetic-circuits";
      });
    } catch {
      results.value = [];
    }
...
async function loadMaterials() {
  try {
    const materialsRes = await api.get("/materials/course/electric-magnetic-circuits");
...
async function joinCourse() {
  ...
  try {
    await api.post("/courses/electric-magnetic-circuits/enroll");
...
function testLink(testId) {
  if (studentPreview.value) {
    return {
      path: `/tests/${testId}`,
      query: { preview: "1", returnTo: "electric-magnetic-circuits" }
    };
  }

  return { path: `/tests/${testId}`, query: { returnTo: "electric-magnetic-circuits" } };
}
...
.electric-magnetic-circuits-course { --electric-orange: #ff5d8f; --electric-blue: #ffb703; }

.electric-magnetic-circuits-course .electronics-grid { opacity: 0.45; transform: perspective(540px) rotateX(56deg); transform-origin: center bottom; }
.electric-magnetic-circuits-course .electronics-shape.ring { border-radius: 50%; border-width: 2px; animation: magnetic-field-spin 9s linear infinite; }
.electric-magnetic-circuits-course .electronics-shape.pulse { border-radius: 50%; animation: circuit-wave-expand 3.6s ease-out infinite; }
@keyframes magnetic-field-spin { to { transform: rotate(405deg) scale(1.08); } }
@keyframes circuit-wave-expand { 0% { transform: scale(0.45); opacity: 0.9; } 100% { transform: scale(1.5); opacity: 0; } }


.electric-magnetic-circuits-course,
.electric-magnetic-circuits-course h1,
.electric-magnetic-circuits-course h2,
.electric-magnetic-circuits-course h3,
.electric-magnetic-circuits-course h4,
.electric-magnetic-circuits-course p,
.electric-magnetic-circuits-course button,
.electric-magnetic-circuits-course a {
  font-family: "Segoe UI", Arial, sans-serif !important;
  letter-spacing: 0 !important;
}
.electric-magnetic-circuits-course .electronics-hero-visual { isolation: isolate; }
.electric-magnetic-circuits-course .board-frame { opacity: 0.16; filter: saturate(0.6); }
.electric-magnetic-circuits-course .course-signature { position: absolute; inset: 10%; z-index: 4; pointer-events: none; }

.electric-magnetic-circuits-course { background: #1c0712; color: #fff2f6; }
.electric-magnetic-circuits-course .course-signature { display: block; inset: 9%; border: 2px solid rgba(255, 183, 3, .72); border-radius: 50%; animation: electric-field-orbit 9s linear infinite; }
.electric-magnetic-circuits-course .course-signature span { position: absolute; inset: calc(10% + 3px); border: 1px solid rgba(255, 93, 143, .72); border-radius: 50%; animation: electric-field-pulse 4.2s ease-out infinite; }
.electric-magnetic-circuits-course .course-signature span:nth-child(3n) { animation-delay: 1.2s; }
.electric-magnetic-circuits-course .course-tabs button, .electric-magnetic-circuits-course .content-panel { border-radius: 14px; }
@keyframes electric-field-orbit { to { transform: rotate(360deg); } }
@keyframes electric-field-pulse { 0% { transform: scale(.42); opacity: .82; } 100% { transform: scale(1.12); opacity: 0; } }
